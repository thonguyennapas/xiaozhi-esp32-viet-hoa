...
void PomodoroApp::BuildUI() {
    ...
    int arc_size = (w < h ? w : h) - 50; // Leave margin
    ...
    // Make arc thick and premium
    lv_obj_set_style_arc_width(arc_, 18, LV_PART_MAIN);
    lv_obj_set_style_arc_color(arc_, lv_color_make(30, 30, 30), LV_PART_MAIN);
    ...
    lv_obj_set_style_text_color(session_label_, lv_color_make(120, 120, 120), 0);

    // Hint / Action button (Bottom)
    lv_obj_t* hint_pill = lv_obj_create(app_root_);
    lv_obj_remove_style_all(hint_pill);
    lv_obj_set_size(hint_pill, 200, 46);
    lv_obj_set_style_radius(hint_pill, 23, 0);
    lv_obj_set_style_bg_opa(hint_pill, LV_OPA_20, 0);
    lv_obj_set_style_bg_color(hint_pill, lv_color_white(), 0);
    lv_obj_align(hint_pill, LV_ALIGN_BOTTOM_MID, 0, -30);
...
void PomodoroApp::UpdateUI() {
    ...
    for (int i = 0; i < SESSIONS_BEFORE_LONG; i++) {
        if (i < sessions_done_) {
            dots[pos++] = '\xe2'; dots[pos++] = '\x97'; dots[pos++] = '\x8f'; // ●
        } else {
            dots[pos++] = '\xe2'; dots[pos++] = '\x97'; dots[pos++] = '\x8b'; // ○
        }
        if (i < SESSIONS_BEFORE_LONG - 1) dots[pos++] = ' ';
    }
    dots[pos] = '\0';
